...
| 5 | Hedge fund: personal savings or multi-investor? | **Resolved in Phase 17: multi-investor.** The domain model supports both: `HedgeFund.investors` dict is present. `FundService.invest(investor_id, fund_id, amount)` is fully implemented — it debits the investor, credits the fund, records the stake, and rejects self-investment. The `/fund invest` command in `FundCog` is fully functional. The APY accrual in `MonthlyRolloverTask` distributes to investors, not just the manager. |
