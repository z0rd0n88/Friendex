...
| phase-8f | [001](./phase-8f/001-2026-05-25-phase-8f-complete.md) | 2026-05-25 | Phase 8f complete — `LiquidationService`, `DisciplineService`, `LiquidationEvent`; `TradingService._cover_internal(force: bool)` extracted (public `cover()` signature unchanged). Gate green: ruff/format/mypy clean, 180 application tests pass, 531 full repo (+9 from 8e baseline), 92.68% application coverage (gate 85%). Design choice (a) declared: helper does NOT acquire locks — caller (public `cover()` or `LiquidationService._maybe_liquidate`) holds them. Discipline uses flat `max(proposed, min_price)` (not `apply_floor_stall`) per spec's flat-percentage definition. Exit digest written. Work uncommitted on `feat/phase-8f-liq-disc` — manager owns commits |
