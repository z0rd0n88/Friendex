...
| phase-8a | [004](./phase-8a/004-2026-05-25-guild-composite-lock-key.md) | 2026-05-25 | HIGH fix applied — composite `f"{guild_id}:{user_id}"` lock key at all 6 call sites in `ActivityService` + `VoicePingService`; new RED-first isolation test (two services sharing one LockManager across different guilds) was load-bearing (timeout RED captured) and now PASSES. Gate green: ruff/format/mypy clean, 430 pytest passed (was 429 + 1 new). 2 LOW findings deliberately deferred to Phase 12 with reviewer rationale + carry-forward to 8b–8f digest |
